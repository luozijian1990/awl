...
不再依赖 macOS 日历导出和 `organizer == null` 过滤——`worklog export report-source` 本身只导出 `status=confirmed` 的工作项，等价于"用户已认可的个人工作"。
...
4. 写出最终 Markdown 到固定输出目录。
   - 默认目录：`/Users/luozijian/Documents/weekly-reports`。
...
- 历史参考（MVP 不调用）：原型用 macOS EventKit 导出 `oa ` 日历 + `organizer==null` 过滤，脚本在 `~/.claude/skills/write-oa-weekly-report/scripts/`。
