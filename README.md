...
Release 产物中，Windows 会同时发布安装包 `Worklog_<version>_x64-setup.exe` 和免安装包 `Worklog_<version>_x64-portable.zip`。portable zip 解压后可直接双击 `Worklog.exe` 运行；如果系统缺少 WebView2 Runtime，请使用安装包。
